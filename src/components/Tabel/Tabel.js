...
import "./Tabel.scss"
import useReecoStore from '../Zustand/ReecoStore';
...
export default function Tabel() {
    const {playData,addData, editedData} = useReecoStore()
    const newUpdateData = {
        productName:"",
        brand:"",
        Price: 0,
    ...
    const [newData,setNewData] = useState(newUpdateData)
  const [additem,setAdditem]= useState(false)
  const [editItem, setEditItem] = useState(false)
  const [editData,setEditdata] = useState()
  const [filterData,setFilterData] = useState(playData)
...
useEffect(()=>{
},[filterData,additem,playData])

const newUpdate = (event)=>{
...
const addItemOk =()=>{
  toast.success("New Item added")
    addData([...playData,newData])
    setFilterData([...playData,newData])
  setAdditem(false)
...
const filterhandler = (event) => {
  ...
  setFilterData(()=>inputValue.length < 1
  ? playData
  : playData.filter((item) => {
    ...
    <Modal open={additem}  onCancel={addItemCancel} onOk={addItemOk}>
      <div className='row'>
        <div className='col-sm-5 col-12'>
            <img src="/Assets/img.jpg" className='img-fluid' />
        </div>
        <div className='col-sm-7 col-12 flex'>
        <div className='d-flex align-items-center'>
            
            <p className='m-0'>Name</p>
            <Input className='rounded-lg my-2 mx-3 ' onChange={newUpdate} value={newData.productName} name='productName'  placeholder='enter name' />
          </div>
          <div className='d-flex align-items-center'>
            
            <p className='m-0'>Brand</p>
            <Input className='rounded-lg my-2 mx-3 input-width' onChange={newUpdate} name="brand" value={newData.brand}  placeholder='enter name' />
          </div>
        <div className='d-flex align-items-center'>
            
                <p className='m-0'>Price</p>
                <Input className='rounded-lg my-2 mx-3 input-width' type='number' onChange={newUpdate} name='Price' value={newData.Price} placeholder='enter Price' />
              </div>
              <div className='d-flex align-items-center'>
                <p className='m-0'>Quantity</p>
                <Input className='rounded-lg my-2 mx-3 input-width' onChange={newUpdate} name='Quantity' value={newData.Quantity} type='number' placeholder='enter Quantity' />
              </div>
